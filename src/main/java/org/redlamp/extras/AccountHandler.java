...
//                }
    ...
    public Map<String, Object> checkLoanEligibility(LoanRequest request)
    {
        String query = "select acct_no from  " + XapiCodes.coreschema + "..ln_display " +
                "where class_code not in (" + XapiPool.allowedLnClass + ") and status ='Active' " +
                "and rim_no = (select b.rim_no from  " + XapiCodes.coreschema + "..gb_user_defined a, "
                + XapiCodes.coreschema + "..dp_acct b  "
                ...
                + "and b.acct_no = a.acct_no_key  and b.acct_no = a.acct_no_key    and b.acct_no = a.acct_no_key  )";
    ...
    {
        CRCaller crCaller = new CRCaller();
        crCaller.setCall("loanApplicationreq", request);
        ...
        else
        {
            ...
            if (isBorrower)
            {
                if (request.getLoanAmount().compareTo(XapiPool.maxBorrowerLnAmount) > 0)
                {
                    limitResponse = EXCEEDS_HIGHER_LIMIT;
                    isLimitExeeded = true;
                }
                if (request.getLoanAmount().compareTo(XapiPool.minBorrowerLnAmount) < 0)
                {
                    limitResponse = LESS_LOWER_LIMIT;
                    isLimitExeeded = true;
                }
            }
            else
            {
                if (request.getLoanAmount().compareTo(XapiPool.maxDepositorLnAmount) > 0)
                {
                    limitResponse = EXCEEDS_HIGHER_LIMIT;
                    isLimitExeeded = true;
                }
                if (request.getLoanAmount().compareTo(XapiPool.minDepositorLnAmount) < 0)
                {
                    limitResponse = LESS_LOWER_LIMIT;
                    isLimitExeeded = true;
                }
            ...
            {
                response.put("responseCode", limitResponse);
            }
        }
    ...
    {
        CRCaller crCaller = new CRCaller();
        crCaller.setCall("checkLoanBalancereq", request);
        setStartTime(System.currentTimeMillis());
        crCaller.setCardNumber(request.getPhoneNumber());
        crCaller.setAccountNo(request.getRepaymentDpAccountNo());
...
//                }
                ...
                {
                    String nextPmtDate = getNextRepaymentDate(loanRepaymentData.getLnAccountNo().trim());
                    map.put("Loan_Bal", payoffBal.setScale(2, 2));
                    map.put("Loan_Account", loanRepaymentData.getLnAccountNo().trim());
                    map.put("Repayment_Date", nextPmtDate);
        ...
        response.put("responseTxt", XapiCodes.getErrorDesc(response.get("responseCode")));
        crCaller.setCall("checkLoanBalanceres", response);
    ...
    {
        ApiLogger.getLogger().info("select   rim_no, acct_no,acct_type,status,class_code  " +
    ...
    public Map<String, Object> repayLoan(LoanRepaymentRequest request)
    {
        ...
        crCaller.setCardNumber(request.getPhoneNumber());
        crCaller.setAccountNo(request.getRepaymentDpAccountNo());
        System.out.println("select  rim_no, acct_no,acct_type,status,class_code  " +
        ...
        {
            crCaller.setCall("loanrepaymentrequest", request);
            crCaller.setAccountNo(loanRepaymentData.getLnAccountNo());
            crCaller.setCardNumber(request.getPhoneNumber());
            crCaller.setCall("earlyPayment", request.getEarlyPayment());
            crCaller.setCall("amount", request.getRepaymentAmount());
            try (CallableStatement callableStatement = conn.prepareCall(SQL.REPAY_LOAN))
            {
                ...
                crCaller.setCall("loanrepaymentresponse", blResponse);
                if (returnCode == 0 || returnCode == 00)
                {
                ...
                else if (returnCode == 39)
                {
                    response.put("responseCode", XapiPool.resolveError(INSUFFICIENT_FUNDS_1));
                    response.put("responseTxt", XapiCodes.getErrorDesc(INSUFFICIENT_FUNDS_1));
                }
...
//            LoanRepaymentData checkBalance = checkLoanRepaymentBalance(request, loanRepaymentData.getLnAccountNo(), false);
//            map.put("Loan_Account", loanRepaymentData.getLnAccountNo());
//            map.put("Loan_Bal", checkBalance.getRepaymentAmount());
//
//            list.add(map);
//            response.put("repayment", list);
//            blResponse.setAmount(checkBalance.getRepaymentAmount());
//        }
        ...
        //response.put("responseTxt", XapiCodes.getErrorDesc(response.get("responseCode")));
        crCaller.setCall("apilnrepaymentresponse", response);
...
//            response.put("class_codes", marketing_types);
    ...
    public String getNextRepaymentDate(String acctNo)
    {
        String local = "";
        ApiLogger.getLogger().info("select mat_dt  from  " + XapiCodes.coreschema + "..ln_display a "
                + "where   a.acct_no  = '" + acctNo + "'  ");
        try (Statement statement = conn.createStatement();
             ResultSet rs = statement.executeQuery("select mat_dt  from  " + XapiCodes.coreschema + "..ln_display a "
                     + "where   a.acct_no  = '" + acctNo + "'  "))
        {
            if (rs.next())
            {
                local = rs.getString("mat_dt");
            }
        } catch (Exception ex)
        {
            ex.printStackTrace();
