...
//                }
    ...
    {
        ApiLogger.getLogger().info("select acct_no from  " + XapiCodes.coreschema + "..ln_display where class_code not in (" + XapiPool.allowedLnClass + ") and rim_no = (select b.rim_no from  " + XapiCodes.coreschema + "..gb_user_defined a,  " + XapiCodes.coreschema + "..dp_acct b, " + XapiCodes.coreschema + "..rm_services c "
        ...
        Map<String, Object> response = new HashMap<String, Object>();
        boolean isBorrower = checkIfExists("select acct_no from  " + XapiCodes.coreschema + "..ln_display where class_code not in (" + XapiPool.allowedLnClass + ") and rim_no = (select b.rim_no from  " + XapiCodes.coreschema + "..gb_user_defined a,  " + XapiCodes.coreschema + "..dp_acct b, " + XapiCodes.coreschema + "..rm_services c "
                + "where a.field_id=45 and   (a.acct_no_key = '" + request.getAccountNo().substring(0, 3) + "-" + request.getAccountNo().substring(3) + "' or value = '" + request.getAccountNo().replace("-", "") + "') "
                + "and b.acct_no = a.acct_no_key  and c.rim_no = b.rim_no and c.cust_service_key = '" + request.getPhoneNumber() + "'  and b.acct_no = a.acct_no_key  and b.rim_no =c.rim_no  and b.acct_no = a.acct_no_key and b.rim_no =c.rim_no and services_id =44)");
